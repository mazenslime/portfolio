...
export const Navbar = ({ Width ,isOpen,Skill,Workme,Aboutme,Contactus }) => {
  ...
  return (
    ...
    className={`${Width >600 ?`flex items-center justify-between p-5 px-20 fixed w-full z-[2000]  ${isScrolled ? 'scrolled bg-#AB7BFF' : ''}`:`h-lvh w-1/2 px-4  flex flex-col justify-center items-start gap-5 bg-black text-white fixed left-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-all duration-300  z-[2000] `}`}>
        ...
        <button className='px-4 py-2 border border-white-500 text-white rounded-3xl cursor-pointer hover:text-white hover:bg-blue-500 transition duration-300 ease-in  ' onClick={()=>{Contactus.current.foucs}}><a href="#contact">Countact Us</a></button>
    </motion.div>
  )
